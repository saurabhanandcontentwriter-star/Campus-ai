...
import { Award, BookOpen, ChevronRight, HelpCircle, Check, X, RotateCcw, AlertCircle, Heart, Sparkles } from 'lucide-react';
import { Student } from '../types';
import AITechnicalInterview from './AITechnicalInterview';

interface VivaVocePrepProps {
  students: Student[];
  onAwardPoints?: (points: number, reason: string) => void;
}

export default function VivaVocePrep({ students, onAwardPoints }: VivaVocePrepProps) {
  const [vivaSubTab, setVivaSubTab] = useState<'mcq' | 'ai'>('ai');
  ...
  return (
    <div className="space-y-6" id="viva_voce_module_root">
      {/* Segmented Sub-tab switcher */}
      <div className="flex gap-2 p-1 bg-slate-200/60 rounded-xl max-w-sm select-none border border-slate-300/10">
        <button
          onClick={() => setVivaSubTab('ai')}
          className={`flex-1 py-2 px-3 rounded-lg text-xs font-black font-mono transition-all duration-200 cursor-pointer flex items-center justify-center gap-1.5 ${
            vivaSubTab === 'ai' 
        ...
        >
          ...
          <span>AI HR Interview</span>
        </button>
        <button
          onClick={() => setVivaSubTab('mcq')}
          className={`flex-1 py-2 px-3 rounded-lg text-xs font-black font-mono transition-all duration-200 cursor-pointer flex items-center justify-center gap-1.5 ${
            vivaSubTab === 'mcq' 
              ? 'bg-slate-900 text-white shadow-sm' 
              : 'text-slate-500 hover:text-slate-800'
        ...
        >
          ...
          <span>Theory MCQs</span>
        </button>
      </div>

      {vivaSubTab === 'ai' ? (
        <AITechnicalInterview students={students} onAwardPoints={onAwardPoints} />
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in animate-duration-200" id="viva_voce_module">
          {/* Quiz Card */}
